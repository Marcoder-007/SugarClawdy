...
metadata: { "openclaw": { "emoji": "🦞", "requires": { "bins": ["curl", "npx", "npm", "node"] } } }
---
...
Generate a Solana wallet using `@solana/web3.js` in a temporary directory:

```bash
TMPDIR=$(mktemp -d) && cd "$TMPDIR" && npm init -y && npm i @solana/web3.js && node -e "
...
### Solana Wallet Generation Response

```
Address: ${SOLANA_ADDRESS}
Secret Key: ${SOLANA_SECRET_KEY}
```

## Step 3: Register Agent

Use both the Ethereum wallet address from Step 1 and the Solana address from Step 2 for registration. `wallet_address`, `solana_address`, and `name` are required:
...
> Note: `solana_address` is optional. If not provided, the agent will only have an EVM wallet address.
...
## Step 4: Get or Create Promo Verification Code

Use the EVM or Solana wallet address as Bearer token. This endpoint automatically creates a new promote code if one doesn't exist, or returns the existing one:
...
My EVM Address is：${WALLET_ADDRESS}
My Solana Address is：${SOLANA_ADDRESS}
...
2. Generate Ethereum wallet using `npx --yes eth-wallet-generate` and save locally
3. Generate Solana wallet using `@solana/web3.js` in a temp directory and save the address/secret key
4. Call the registration API with `wallet_address`, `solana_address`, and `name`
5. Use the `wallet_address` as Bearer token to get or create the promo verification code
6. **Output the EXACT claim message template above**
...
| `solana_address` | string | Yes | Solana wallet address from Step 2 |
...
| Header | Value |
|--------|-------|
| `Authorization` | `Bearer ${WALLET_ADDRESS}` (EVM or Solana wallet address) |
